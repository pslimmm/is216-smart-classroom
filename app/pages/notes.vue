<template>
    <div class="container py-5 d-flex justify-content-center flex-column">
        ...
        <!-- Clear All Button (Testing Only) - PROF/TA ONLY -->
        <!-- COMMENTED OUT FOR PRODUCTION - TESTING ONLY -->
        <!-- <div v-if="canRecord" class="text-center mt-4">
            <button @click="clearAllTranscripts" class="btn btn-danger">
                <i class="bi bi-trash me-2"></i>
                Clear All Transcripts (Testing Only)
            </button>
        </div> -->
    </div>
</template>
...
const loadRecordings = async () => {
    try {
        ...
        console.log('Loaded recordings:', recordings?.length || 0);
...
const transcribeAudio = async () => {
    ...
    try {
        ...
        // - Storage upload errors: "Failed to upload audio: ..."
        // - Processing errors: "Failed to process recording"
        // Failed uploads are saved to DB and can be retried from the UI
...
// ============================================
// Unused - Test production only: Delete all recordings from Supabase (DB + Storage)
// ============================================
const clearAllTranscripts = async () => {
    ...
    try {
        ...
        // Step 1: Fetch all recordings to get audio paths
        ...
        if (fetchError) {
            ...
            errorMessage.value = 'Failed to fetch recordings';
            return;
        }

        // Step 2: Delete audio files from Storage
        if (recordings && recordings.length > 0) {
            const audioPaths = recordings.map(r => r.audio_path);
            ...
            if (storageError) {
                ...
                errorMessage.value = 'Failed to delete audio files';
                return;
            }

            console.log(`Deleted ${audioPaths.length} audio files from storage`);
        }

        // Step 3: Delete all recordings from database
        const { error: dbError } = await supabase
            .from('recordings')
            .delete()
        ...
        if (dbError) {
            console.error('Error deleting recordings from DB:', dbError);
            errorMessage.value = 'Failed to delete recordings from database';
            return;
        }
        ...
        successMessage.value = 'All transcripts and audio files cleared!';
        setTimeout(() => {
            successMessage.value = '';
        }, 3000);

    } catch (error) {
        console.error('Clear transcripts error:', error);
        errorMessage.value = 'Failed to clear transcripts';
    }
};
</script>
