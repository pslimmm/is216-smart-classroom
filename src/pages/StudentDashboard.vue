...
export default {
  ...
  data() {
    return {
      ...
      enrolledCourses: ["Math 101", "Physics 202"], // List of enrolled courses
      coins: 125, // Total coins for marketplace purchases (managed by Chun Wai)
  ...
  computed: {
     ...
     */
    totalParticipations() {
      return this.participationData.myWeeklyCount.reduce((sum, count) => sum + count, 0);
    },
     ...
     */
    myOverallAvgRating() {
      const ratings = this.participationData.myAvgRating.filter(r => r > 0);
     ...
     */
    classOverallAvgRating() {
      const ratings = this.participationData.classAvgRating.filter(r => r > 0);
      return ratings.reduce((sum, r) => sum + r, 0) / ratings.length;
    },
     ...
     */
    currentWeekParticipations() {
      return this.participationData.myWeeklyCount[this.currentWeek - 1] || 0;
    },
     ...
     */
    gradeProjection() {
      const avgParticipationsPerWeek = this.totalParticipations / this.currentWeek;
      if (avgParticipationsPerWeek >= 3 && this.myOverallAvgRating >= 4.0) return "A+";
      if (avgParticipationsPerWeek >= 3 && this.myOverallAvgRating >= 3.5) return "A";
     ...
     */
    weeklyAlertClass() {
      if (this.currentWeekParticipations >= 3) return 'alert-success';
      if (this.currentWeekParticipations >= 2) return 'alert-warning';
      return 'alert-danger';
    }
  },
   ...
   */
  mounted() {
    // Route protection: Redirect non-students to home
   ...
   * Destroys Chart.js instances to prevent memory leaks
   */
  beforeUnmount() {
  ...
  methods: {
    ...
    createQualityChart() {
      ...
      this.qualityChart = new Chart(ctx, {
        ...
        data: {
          labels: this.participationData.weeks, // X-axis: Week numbers
          datasets: [
            {
              label: 'My Avg Rating',
              data: this.participationData.myAvgRating,
            ...
            },
            {
              ...
              data: this.participationData.classAvgRating,
    ...
    createCountChart() {
      ...
      const counts = this.participationData.myWeeklyCount;
